...
4. Define workspaces for each Worker:
...
6. Add the Worker host as a Next.js public environment variable:
...
<Steps>
1. In your `wrangler.toml` file, declare the external Durable Object binding and the Service binding to `SessionsRPC`:
...
2. Update your `env.d.ts` file for typesafety:
	```ts title="env.d.ts" {2-5}
...
3. Include Next.js server side logic:
	...
	```tsx title="src/app/page.tsx" {9-10,19,25-27,33}
